    ...
    for y in range(col_n-1):
        try:
            if len(data[0][x][y]) == 0:
                continue
            #sample_value = random.randint(0,4);
            print("%f at %d,%d"%(data[0][x][y][5], x, y))
            grid2[x][y] = float(data[0][x][y][5])#grid[x][y][1] #float(arr2[x][y][1:(len(arr2[x][y])-2)].split(',')[sample_value])
            print(grid2[x][y])
            #if x+y >= 100:
             #   grid2[x][y] = float('NaN')
            #if grid2[x][y] < 0:
...
im = ax.imshow(masked_array, interpolation='none', cmap=colormap, extent=[0,1,1,0])
